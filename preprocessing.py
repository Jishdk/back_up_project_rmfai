...
class DatasetPreprocessor:
    ...
    def process_image_batch(self, image_paths: List[Path], labels: List[str],
        ...
        for i in tqdm(range(0, total_images, CHUNK_SIZE), desc="Processing image chunks"):
            chunk_paths = image_paths[i:i + CHUNK_SIZE]
            chunk_labels = labels[i:i + CHUNK_SIZE]

            for img_path, label in zip(chunk_paths, chunk_labels):
                try:
                ...
                # Process chunk when it reaches a certain size to avoid memory issues
                if len(processed_images) >= CHUNK_SIZE * 2:  # Including augmented images

...
def calculate_metrics(results: object) -> Dict:
...
def calculate_per_class_metrics(results: object, class_names: List[str]) -> Dict:
    """Calculate precision, recall, and mAP for each class
    ...
    for i, name in enumerate(class_names):
        metrics[name] = {
            'precision': float(results.results_dict[f'metrics/precision({i})']),
            'recall': float(results.results_dict[f'metrics/recall({i})']),
            'mAP50': float(results.results_dict[f'metrics/mAP50({i})']),
            'mAP50-95': float(results.results_dict[f'metrics/mAP50-95({i})'])
        }
    return metrics
...
def plot_class_performance(metrics: Dict, save_path: Path, 
    ...
    metrics_df[METRICS].plot(kind='bar')
    plt.title(title)
    plt.xlabel('Class')
    plt.ylabel('Score')
    plt.legend()
